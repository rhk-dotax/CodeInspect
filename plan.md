...
- [x] **분석 결과 UI 표시 수정** — WinForms 도킹 레이아웃 문제로 dgvResults 가용 영역이 비정상 축소되던 문제 해결. pnlRight 컨테이너 Panel 도입으로 pnlSummary/dgvResults를 올바른 영역에 배치 — v1.6.1
- [x] **개별 파일 수동 분석** — 좌측 패널에 "📄 단일 파일 분석" 버튼 추가. OpenFileDialog로 1개 이상의 소스 파일을 골라 즉시 분석하며, 룰셋/LLM 모드/취소/그리드/CSV/로그 인프라를 모두 재사용. 미지원 확장자는 사전에 감지하여 자동 제외 또는 안내 — v1.7
